...
class Resources:
    ...
    def loadResources(self):
        ...
        stream = open(self.common.getInputFile(), encoding="utf-8-sig")
        ...
        for resource in self.resourceDictionary:
            ...
            frame = pd.DataFrame.from_dict(table, orient="index") if table != {} else pd.DataFrame()
    ...
    def combineEndpointGatewayIP(self):
        ...
        for _ipindex, iprow in ipresource.iterrows():
            for _mainkey, mainvalue in maintable.items():
                mainid = mainvalue["id"]
                if mainid == iprow["gateway"]:
                    for _subnetindex, subnetrow in subnetresource.iterrows():
        ...
        for _mainkey, mainvalue in maintable.items():
            ...
            for zone in mainzones:
                newvalue = {
                    ...
                    "name": mainname,
                    "vpc_id": mainvpc,
                    "zones": [{"name": zone["name"], "subnet_id": zone["subnet_id"]}],
                }
                if newtable == {}:
