...
class Build:
    ...
    def buildGroupShape(self, groupid, properties):
        ...
        shape = properties["shape"].lower()  # noqa: F841
    ...
    def buildConnectorShape(self, connectorid, properties):
      ...
      """
        ...
        if startarrow == "" and endarrow == "":
            connectornode = self.shapes.buildSolidLink(
                connectorid,
                label,
                sourceid,
                targetid,
    ...
    def checkGroups(self, groups):
        for groupid, properties in groups.items():
            ...
            hideicon = False if properties["hideicon"] == "" else properties["hideicon"]
    ...
    def checkConnectors(self, connectors):
        for connectorid, properties in connectors.items():
            linecolor = properties["linecolor"]
            if linecolor == "":
                hexcolor = self.checkLineColor(linecolor)  # noqa: F841
                self.common.printInvalidLineColor(linecolor)
                return None
            # else:
    ...
    def addKeys(self):
        ...
        for groupid, _properties in self.groups.items():
        ...
        for nodeid, _properties in self.items.items():
    ...
    def calculateLeftRightGroups(self, parentid):
        ...
        minnodewidth = 48  # noqa: F841
        minnodeheight = 48  # noqa: F841
        mingroupwidth = 240  # noqa: F841
        mingroupheight = 152  # noqa: F841
        ...
        parentdirection = parent["direction"]  # noqa: F841
        parentfinal = parent["final"]  # noqa: F841
        ...
        for childids in parentchildren:
            ...
            childx = childgeometry[0]  # noqa: F841
            childy = childgeometry[1]  # noqa: F841
    ...
    def calculateTopBottomGroups(self, parentid):
        ...
        parent = self.groups[parentid]
        parentgeometry = parent["geometry"]
        parentx = parentgeometry[0]  # noqa: F841
        parenty = parentgeometry[1]  # noqa: F841
        parentwidth = parentgeometry[2]
        parentheight = parentgeometry[3]
        parentchildren = parent["children"]
        parentdirection = parent["direction"]  # noqa: F841
    ...
    def calculateLeftRightItems(self, parentid, parentgeometry):
        ...
        for childids in parentchildren:
            ...
            if isinstance(childids, list):
                ...
                minimalwidth = (
                    (2 * minshapespace) + (nodecount * minnodeheight) + (nodecount * minitemspace)
                )
                ...
                adjustwidth = max(
                    adjustwidth,
                    minitemspace + (nodeindex * minnodewidth) + (nodeindex * minitemspace),
                )
        ...
        for childids in parentchildren:
            ...
            if isinstance(childids, list):
                ...
                minimalheight = (
                    mintopspace
                    + minshapespace
                    + (nodecount * minnodeheight)
                    + (nodecount * minitemspace)
                )
                ...
                adjustheight = max(
                    adjustheight,
                    mintopspace + (nodeindex * minnodeheight) + (nodeindex * minitemspace),
                )
        ...
        for bottomid in self.bottoms:
            ...
            while parentid is not None:
                ...
                if zonesFound:
                    ...
                    direction = childproperties["direction"]  # noqa: F841
   ...
   """
    ...
    def printSequence(self):
        for sequenceid in self.sequence:
            if sequenceid in self.diagrams:
                ...
                label = "None"
            logger.debug("")
            logger.debug('"%s": %s', sequenceid, label)
        return

    def printDiagrams(self):
        logger.debug("")
        logger.debug("Diagrams:")
    ...
    def getZoneCIDR(self, zone):
        match zone:
            case "au-syd-1":
                cidr = ZoneCIDR.AU_SYD_1
            case "au-syd-2":
                cidr = ZoneCIDR.AU_SYD_2
            case "au-syd-3":
                cidr = ZoneCIDR.AU_SYD_3

            case "br-sao-1":
                cidr = ZoneCIDR.BR_SAO_1
            case "br-sao-2":
                cidr = ZoneCIDR.BR_SAO_2
            case "br-sao-3":
                cidr = ZoneCIDR.BR_SAO_3

            case "ca-tor-1":
                cidr = ZoneCIDR.CA_TOR_1
            case "ca-tor-2":
                cidr = ZoneCIDR.CA_TOR_2
            case "ca-tor-3":
                cidr = ZoneCIDR.CA_TOR_3

            case "eu-de-1":
                cidr = ZoneCIDR.EU_DE_1
            case "eu-de-2":
                cidr = ZoneCIDR.EU_DE_2
            case "eu-de-3":
                cidr = ZoneCIDR.EU_DE_3

            case "eu-gb-1":
                cidr = ZoneCIDR.EU_GB_1
            case "eu-gb-2":
                cidr = ZoneCIDR.EU_GB_2
            case "eu-gb-3":
                cidr = ZoneCIDR.EU_GB_3

            case "jp-osa-1":
                cidr = ZoneCIDR.JP_OSA_1
            case "jp-osa-2":
                cidr = ZoneCIDR.JP_OSA_2
            case "jp-osa-3":
                cidr = ZoneCIDR.JP_OSA_3

            case "jp-tok-1":
                cidr = ZoneCIDR.JP_TOK_1
            case "jp-tok-2":
                cidr = ZoneCIDR.JP_TOK_2
            case "jp-tok-3":
                cidr = ZoneCIDR.JP_TOK_3

            case "us-east-1":
                cidr = ZoneCIDR.US_EAST_1
            case "us-east-2":
                cidr = ZoneCIDR.US_EAST_2
            case "us-east-3":
                cidr = ZoneCIDR.US_EAST_3

            case "us-south-1":
                cidr = ZoneCIDR.US_SOUTH_1
            case "us-south-2":
                cidr = ZoneCIDR.US_SOUTH_2
            case "us-south-3":
                cidr = ZoneCIDR.US_SOUTH_3
