...
from .ibmbase.build import Build as Build
from .ibmbase.common import Common as Common
from .ibmbase.compose import Compose as Compose
from .ibmbase.load import Load as Load
...
class Diagram:
    ...
    def __init__(
        ...
        self.properties = _data.getDiagramProperties(
            name=name, filename=filename, output=output, fontname=self.fontname, direction=direction
        )
...
class Group:
    ...
    def __sub__(self, shape=None):
        ...
        if isinstance(shape, Group | Item):
            connector = Connector(  # noqa: F841
                sourceid=self.shapeid,
                targetid=shape.shapeid,
                startarrow="",
                endarrow="",
                operator="sub",
            )
...
class Item:
    ...
    def __init__(
        self, label="", sublabel="", linecolor="", fillcolor="", shape="", icon="", hideicon=""
    ):
        # many = False,
...
class Connector:
    ...
    def __init__(
        ...
        if endarrow != "":
            if endarrow.upper() not in [parm.value for parm in EndTypes]:
                logger.error("Connector.__init__: endarrow not supported: %s", endarrow)
                sys_exit()
    ...
    def __sub__(self, shape=None):
        ...
        if isinstance(shape, Group | Item):
            if self.sourceid is not None:
                ...
                _data.setConnectorEndFill(self.shapeid, self.endfill)
                _data.setConnectorOperator(self.shapeid, self.operator)
                logger.error("Connector.__sub__: shape << connector - shape not supported")
                sys_exit()
        else:
            logger.error("Connector.__sub__: connector - shape not supported")
            sys_exit()
    ...
    def __lshift__(self, shape=None):
        ...
        if isinstance(shape, Group | Item):
            ...
            logger.error("Connector.__lshift__: connector << shape not supported")
            sys_exit()
        return shape
    ...
    def __rshift__(self, shape=None):
        ...
        if isinstance(shape, Group | Item):
            ...
            logger.error("Connector.__rshift__: connector >> shape not supported")
            sys_exit()
        return shape
